@forelse($chatDetails as $chatDetail)
    ...
    <li class="@if($chatDetail->from == $user->id) odd @else  @endif">
        <div class="chat-image"> <img alt="user" src="@if(is_null($chatDetail->fromUser->image)){{asset('default-profile-2.png')}} @else{{asset('storage/avatar/'.$chatDetail->fromUser->image)}}@endif"> </div>
        <div class="chat-body">
            <div class="chat-text">
